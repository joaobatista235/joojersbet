...
async function handleProcess() {
  ...
  try {
    ...
    // ===========================================================
    const [syncedCount, ufcSyncedCount, seasonId] = await Promise.all([
      syncMatchesFromApi(),
      syncUfcFights(),
      ensureActiveSeason(),
    ]);
    console.log(`[score/process] Synced ${syncedCount} football matches, ${ufcSyncedCount} UFC fights, season: ${seasonId}`);
    ...
    console.log(`[score/process] Done — Football: ${totalProcessed} matches, ${affectedUsers.size} users | UFC: ${ufcProcessed} fights, ${ufcAffectedUsers.size} users`);
